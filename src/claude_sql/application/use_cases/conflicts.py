...
Moved here from ``analytics/conflicts_worker.py`` in the v2 hexagonal
reshape (MIGRATION Phase C). All Bedrock plumbing -- client construction,
retry, structured-output parsing, the per-pipeline cache-stat accumulator --
lives in :mod:`claude_sql.infrastructure.bedrock.client`.
"""
...
from claude_sql.application.prompts import CONFLICTS_SYSTEM_PROMPT
...
from claude_sql.infrastructure.bedrock.structured_output import SESSION_CONFLICTS_SCHEMA
...
async def _conflicts_async(
    ...
    *,
    ...
    # an injected reader owns the connection.
    bounds = (
...
def detect_conflicts(
    ...
    *,
    ...
    cache = cache if cache is not None else build_cache(settings.conflicts_parquet_path)
    if dry_run:
        already: set[str] = set()
        done_df = cache.read_all(columns=["session_id"])
        if done_df is not None and done_df.height > 0:
