...
def table_identity(lance_uri: Path) -> tuple[str, int] | None:
    ...
    ``None`` means the store is empty or absent (fresh install), so any provider
    may claim it. Every row stamps the same ``model`` / ``dim`` (see
    ``run_backfill``), so reading the first row is sufficient. Used by the
    fail-loud provider/dimension guard
    (:func:`claude_sql.domain.embedding_guard.ensure_store_matches`).
    """
