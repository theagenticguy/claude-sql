...
# Glob constants
# ---------------------------------------------------------------------------
# Derived from ``config._claude_config_root()`` so they honor
# ``CLAUDE_CONFIG_DIR`` (Claude Code's own relocation env var) at import time.
# ``register_raw`` re-resolves via the same factories at call time, so a
# process that re-points ``CLAUDE_CONFIG_DIR`` after import still gets the new
# root; these module names remain importable as documented fallbacks.

DEFAULT_GLOB: str = _default_glob()
SUBAGENT_GLOB: str = _default_subagent_glob()
SUBAGENT_META_GLOB: str = _default_subagent_meta_glob()
...
def register_raw(
    ...
    *,
    ...
    """
    # Re-resolve defaults at call time (not import time) so a process that
    # re-points ``CLAUDE_CONFIG_DIR`` still binds against the current root.
...
def register_vss(
    ...
    *,
    ...
    dim
        Fixed-length embedding dimension for the empty-fallback view. When a
        populated Lance store is present its own stamped ``dim`` is used for the
        ``CAST(embedding AS FLOAT[dim])`` view instead, so a store written by a
        different-width provider (e.g. bge-small at 384 vs the Cohere 1024
        default) binds correctly regardless of this argument.
    metric
    ...
    expected_model
        ...
        ``model_id``. Read against the store's stamped ``model`` column via the
        fail-loud provider guard: a mismatch raises
        :class:`claude_sql.domain.errors.EmbeddingProviderMismatch` rather than
        letting a cross-provider query silently return garbage cosine scores.
    expected_dim
        The active embedder's dimension, checked alongside ``expected_model``.
        ``None`` trusts ``model_id`` alone (probe-only local providers).
    ...
    # Fail-loud provider/dimension guard: the store stamps ``(model, dim)`` on
    # every row; read it back and refuse to bind a view over vectors written by
    # a different provider (even at matching width, cross-model vectors live in
    # incompatible spaces and produce numerically valid but garbage cosine
    # scores). The stored width also drives the CAST below, so a store written
    # by a different-dim provider binds correctly regardless of the ``dim`` arg.
    identity = lance_store.table_identity(lance_uri)
