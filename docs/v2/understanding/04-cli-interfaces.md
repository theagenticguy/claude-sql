...
**`app/install_source.py`** (78 LOC) — install-source discovery for `--version`. A user's `claude-sql` comes from the published wheel (`uv tool install claude-sql`), a local checkout (`--from .`), a git URL, or the project venv, and uv records which in `$UV_TOOL_DIR/<tool>/uv-receipt.toml`. `read_install_source` parses that receipt — a **registry** install is identified by the absence of a `directory` / `url` / `git` key, so it is named explicitly rather than falling through to the unknown-source placeholder (all reads wrapped in try/except — uv's schema is not a public contract, `10-13`); `format_version` (`66-78`) returns `"claude-sql X.Y.Z"` plus a source line, and is passed as `App(version=...)` at `cli.py:170`. `__version__` comes from `importlib.metadata.version("claude-sql")` (`22`). v2: this stays an interface concern; keep it beside the CLI adapter.
