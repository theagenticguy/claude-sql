...
Items 1–4 of the original register are fixed and removed from the ranked
table below (ranks preserved for cross-reference):

| Rank | Debt item | Resolution |
|---|---|---|
| 1 | Banned-api message pointed at nonexistent `claude_sql.core.logging_setup` | Fixed — `pyproject.toml` hint now names `claude_sql.infrastructure.logging_setup` |
| 2 | `domain/costs.py` docstring claimed a live `core.llm_shared` back-compat shim | Fixed — docstring now says the module was deleted in the v2 reshape |
| 3 | `domain/friction.py` docstring claimed a live `analytics.friction_worker` shim | Fixed — docstring now says the shim is gone |
| 4 | pyproject described the "transitional `core`" package in the present tense | Fixed — build-backend comment rewritten; also cleaned the stale lefthook.yml fence comment and the mise.toml `lint:imports` description |

The same pass removed the dead `ANALYTICS_VIEW_NAMES` export (see
[dead-code.md](../analysis/dead-code.md)).
...
### Stale references to deleted packages — RESOLVED 2026-07-20

The reshape deleted `core/`, `analytics/`, and `app/`, but a scatter of docstrings, comments, and one manifest string still named them as if they were live (the banned-api hint pointing at a deleted module path, domain docstrings claiming back-compat shims that were removed, present-tense "transitional `core`" comments in pyproject/lefthook/mise). All sites are fixed; the remaining `core` mentions in source are past-tense historical records ("dissolved in T-8-2"), which are accurate.
